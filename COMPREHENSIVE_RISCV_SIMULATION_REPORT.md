...
### 深入解析：為什麼不同架構的測資與 Cycle Count 標準不同？ (Fairness & Evaluation)

在比較不同架構時，初學者常會有個疑問：「**為什麼不讓所有架構跑一模一樣的 256 個 Seed，這樣比較才公平？**」或者「**為什麼同樣的 Pure C Golden Model，在不同架構模擬時的 Cycle Count 會不一樣？**」

這其實牽涉到**硬體架構設計初衷**與**底層微架構模擬**的兩個核心觀念：

#### 1. 測資不同：為了讓不同架構發揮其「設計初衷」(SIMD vs. 專用 ASIC)
在計算機結構基準測試中，真正的公平是**「讓硬體發揮它被設計出來要解決的最佳化場景」**：
* **純量專用加速器 (RoCC / DMA / MMIO)**：
  這類 ASIC 設計邏輯是「**深度循序運算 (Deep Sequential)**」。它們內部只有單一資料通道，擅長吞下一個 Seed 然後用最快速度狂轉 10,000 步。如果強迫它們處理 256 個不同的 Seed，CPU 必須寫迴圈連續喚醒硬體 256 次，CPU 與硬體之間的溝通延遲 (Overhead) 會徹底吃掉硬體省下的時間。
* **向量擴充處理器 (RVV)**：
  RVV 的設計邏輯是「**廣度平行運算 (Wide Parallel - SIMD)**」。它具備超寬的向量暫存器，一次可以平行處理上百個 Seed。如果強迫 RVV 只處理 1 個 Seed 跑 10,000 步，會導致其 99% 的硬體運算單元都在閒置，這完全違背了向量單元處理陣列資料的初衷。

這也是為何 RVV 的 SW Cycle Count (約 20 萬) 會大幅高於其他架構 (約 8 萬)，因為 RVV 的測試實際上包含了 `256 x 100 = 25,600` 次的 LFSR 運算，是其他測試 (`1 x 10,000`) 運算量的 2.56 倍。不同的測資，旨在展示該硬體在面臨其最擅長的資料型態 (向量陣列平行 vs 單筆資料深度) 時所能爆發的加速潛力。

#### 2. Golden Cycle Count 不同：底層微架構與 SoC 配置的差異
儘管各個 Demo 中純軟體 (`lfsr_sw`) 的 C 語言程式碼 100% 相同，但在模擬環境下測出的 Cycle Count 卻會有微小浮動 (例如 80,043 vs 90,025)：
* **SoC 硬體配置 (Config) 不同**：載入 DMA 與載入 RoCC 的 Rocket Chip 在底層 TileLink 匯流排的節點與 Crossbar 拓樸會有所不同。這種硬體層級的微小改變，會影響 CPU 在快取未命中時去 Main Memory 抓資料的延遲。
* **快取未命中與記憶體對齊 (Cache Miss & Alignment)**：由於各個專案編譯出的二進位執行檔大小與結構改變，C 語言迴圈指令在實體主記憶體中的擺放位址會不同，進而導致 L1 Instruction Cache 的 Hit/Miss 命中率及 Branch Predictor 分支預測的狀態產生微小變化。
* **精確的硬體計數器 (`mcycle`)**：因為我們使用的是 RTL 模擬器，它是 Cycle-Accurate (精確到每個時鐘週期) 的。任何因 Pipeline Stall 或是 Cache 造成的微小等待都會被如實記錄，這也是為什麼它能真實反映晶片出廠後效能的原因！
