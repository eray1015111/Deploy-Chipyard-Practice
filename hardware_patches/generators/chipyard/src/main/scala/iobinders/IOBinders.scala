...
class WithMACIOPunchthrough extends OverrideIOBinder({
  (system: CanHavePeripheryMAC) => {
    val macBusyPort = system.mac_busy.map { busy =>
      val io_mac_busy = IO(Output(Bool())).suggestName("mac_busy")
      io_mac_busy := busy
      MACBusyPort(() => io_mac_busy)
    }.toSeq
    (macBusyPort, Nil)
  }
})
