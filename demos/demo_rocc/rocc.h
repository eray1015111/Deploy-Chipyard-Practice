// Based on the Rocket Chip RoCC example macros.
...
#define ROCC_INSTRUCTION_R_R_R(X, rd, rs1, rs2, funct, rd_n, rs1_n, rs2_n) do { \
...
#define ROCC_INSTRUCTION_R_R_I(X, rd, rs1, rs2, funct, rd_n, rs1_n) do { \
    register uint64_t rd_ asm("x" # rd_n); \
    register uint64_t rs1_ asm("x" # rs1_n) = (uint64_t)(rs1); \
    asm volatile ( \
...
#define ROCC_INSTRUCTION_R_I_I(X, rd, rs1, rs2, funct, rd_n) do { \
    register uint64_t rd_ asm("x" # rd_n); \
    asm volatile ( \
...
#define ROCC_INSTRUCTION_I_R_R(X, rd, rs1, rs2, funct, rs1_n, rs2_n) do { \
    register uint64_t rs1_ asm("x" # rs1_n) = (uint64_t)(rs1); \
    register uint64_t rs2_ asm("x" # rs2_n) = (uint64_t)(rs2); \
    asm volatile ( \
...
#define ROCC_INSTRUCTION_I_R_I(X, rd, rs1, rs2, funct, rs1_n) do { \
    register uint64_t rs1_ asm("x" # rs1_n) = (uint64_t)(rs1); \
    asm volatile ( \
...
#define ROCC_INSTRUCTION_I_I_I(X, rd, rs1, rs2, funct) do { \
    asm volatile ( \
